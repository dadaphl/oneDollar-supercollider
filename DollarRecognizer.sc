DollarRecognizer {
	...
	*new {
	...
	recognize { arg points, useProtractor;
		...
		points = thisresample(points, NumPoints);
		radians = thisindicativeAngle(points);
		points = thisrotateBy(points, -1 * radians);
		points = thisscaleTo(points, this.squareSize);
		points = thistranslateTo(points, Origin);
		vector = thisvectorize(points); // for Protractor

		this.unistrokes.do({ arg unistroke, i;
			var d;
			if (useProtractor, {
				d = OptimalCosineDistance(this.unistrokes[i].vector, vector);
			},{
				d = DistanceAtBestAngle(points, this.unistrokes[i], -1 * this.angleRange, this.angleRange, this.anglePrecision);
			}); // for Protractor
			if (d < b, {
		...
		if (u == -1,{
			^Result.new("No match.", 0.0);
		}, {
			^Result.new(this.unistrokes[u].name, if(useProtractor,{ 1.0 / b}, { 1.0 - b / HalfDiagonal}));
		});
	}

	addgesture { arg name, points;
		var num = 0;
		this.unistrokes[this.unistrokes.size] = Unistroke.new(name, points); // append unistroke.new
		this.unistrokes.do({ arg unistroke, i;
